### Methods for generating computationally valid meshes. Idealy one should use Meshes.jl and fix it's topology with some method like fixtopology(vertices,normals,faces). 
...
function subdivide(msh::HomogenousMesh,f::Function)
    ...
    edges = filter(x->x[1]<x[2],decompose(Face{2,Int},msh)) ### Since subdivission does not work at the moment with other topology types, no need to generalize that.
